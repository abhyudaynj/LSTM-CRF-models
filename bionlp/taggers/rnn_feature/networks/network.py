...
def setup_NN(worker,x_in,u_in,mask_in,y_in,params,numTags,emb_w):
    ...
    backward1= lasagne.layers.LSTMLayer(dropout_backward,params['hidden1'],mask_input=l_mask,peepholes=False,forgetgate=lasagne.layers.Gate(b=lasagne.init.Constant(1.)),nonlinearity=lasagne.nonlinearities.tanh,backwards=True,precompute_input=True)
    forward1= lasagne.layers.LSTMLayer(dropout_forward,params['hidden1'],mask_input=l_mask,peepholes=False,forgetgate=lasagne.layers.Gate(b=lasagne.init.Constant(1.)),nonlinearity=lasagne.nonlinearities.tanh,precompute_input=True)
    ...
    dense1=lasagne.layers.DenseLayer(dropout1,numTags,nonlinearity=output_nonlinearity)
    ...
    logger.info("output shape for for unary potential layer {0}".format(mid_output(x_in.astype('int32'),u_in.astype('float32'),mask_in.astype('float32')).shape))


    sum_layer=CRFLayer(crf_layer)
    crf_params=sum_layer.get_CRF_params()
    logger.info('\'l2crf\' Regularization is applied to only to trainable parameter \'W_p\' in CRF')
    ...
    crf_train_loss=get_crf_training_loss(sum_layer,t_out,numTags,params,x_in,u_in,y_in,mask_in,l_in,l_u_in,l_mask)
