...
class DualCRFLayer(lasagne.layers.MergeLayer):
    def __init__(self, incomings, mask_input=None, **kwargs):
        ...
        self.num_labels = incomings[0].output_shape[-1]
        if mask_input == True:
            self.mask_input = True
        else:
            assert False, 'Output without Mask is not yet Enabled'
        assert incomings.__len__(
        ) == 3, 'the CRF layer should have 3 inputs: unary_potential,pairwise_potential and mask'
    ...
    def get_output_for(self, inputs, normalization=False, **kwargs):
        ...
        pairwise_sequence = T.reshape(
            pairwise_sequence, (pair_shape[0], pair_shape[1], self.num_labels, self.num_labels))
        ...
        def forward_scan1(unary_sequence, Wp, mask, forward_sm):
            ...
            forward_sm = theano_logsumexp(forward_sm.dimshuffle(
                0, 1, 'x') + mask.dimshuffle(0, 'x', 'x') * Wp, 1)
            return forward_sm

        def backward_scan1(unary_sequence, Wp, mask, forward_sm):
            forward_sm = forward_sm + unary_sequence * mask.dimshuffle(0, 'x')
            forward_sm = theano_logsumexp(forward_sm.dimshuffle(
                0, 1, 'x') + mask.dimshuffle(0, 'x', 'x') * Wp.dimshuffle(0, 2, 1), 1)
            return forward_sm

        forward_results, _ = theano.scan(fn=forward_scan1, sequences=[unary_sequence, pairwise_sequence, mask], outputs_info=T.zeros_like(
            unary_sequence[0]), non_sequences=None, n_steps=unary_sequence.shape[0] - 1)
        backward_results, _ = theano.scan(fn=backward_scan1, sequences=[unary_sequence[::-1], pairwise_sequence[::-1], mask[::-1]], outputs_info=T.zeros_like(
            unary_sequence[0]), non_sequences=None, n_steps=unary_sequence.shape[0] - 1)
...
def get_crf_training_loss(Inference_Layer, pairwise, t_out, numTags, params, x_in, u_in, y_in, mask_in, l_in, l_u_in, l_mask):
    ...
    logger.info("rearranged output shape for mask {0}".format(
    ...
    pairwise_score_result = theano.function(
        [t_out, l_in.input_var, l_u_in.input_var, l_mask.input_var], pairwise_score)
    logger.debug('Scan pairwise score is calculated.{0}'.format(pairwise_score_result(y_in.astype(
        'float32'), x_in.astype('int32'), u_in.astype('float32'), mask_in.astype('float32')).shape))
    ...
    up_ = theano.function(
        [l_in.input_var, l_u_in.input_var, l_mask.input_var], unnormalized_log)
    ...
    logger.debug('Unary potential score is calculated.The vector size {0} should be the same as the batch size.'.format(
        up_(x_in.astype('int32'), u_in.astype('float32'), mask_in.astype('float32')).shape))
    ...
    visible_zee = zee_(x_in.astype('int32'), u_in.astype(
        'float32'), mask_in.astype('float32'))
    logger.debug('Zee score shape is calculated.The vector size {0} should be the same as the batch size.'.format(
        visible_zee.shape))
    logger.debug('Zee score is calculated for sample 0. Zee should be the same for all label instances. \nZee :{0} \nMask: {1}'.format(
        visible_zee[0, :][mask_in[0] != 0], mask_in[0]))
    ...
    logger.debug('Zee score is calculated for sample 10. Zee should be the same for all label instances. \nZee :{0} \nMask: {1}'.format(
        visible_zee[10, :][mask_in[10] != 0], mask_in[10]))
